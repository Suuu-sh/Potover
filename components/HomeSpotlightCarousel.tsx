...
export function HomeSpotlightCarousel(){
  ...
  useEffect(()=>{
    ...
    const timer=window.setInterval(()=>{const node=rail.current;if(!node)return;const step=stepFor(node);const cycle=cycleWidthFor(node);const next=node.scrollLeft+step;node.scrollTo({left:next,behavior:'smooth'});if(cycle&&next>=cycle*2-step/2)window.setTimeout(()=>{if(node.scrollLeft>=cycle*2-step/2)node.scrollLeft-=cycle},650)},4000);
    return()=>window.clearInterval(timer);
  },[paused,preferredLanguage]);
