...
export function RoadmapPreview(){
  ...
  return <section className="roadmap-preview"><div className="modern-section-head"><div><small>LEARNING PATHS</small><h2>学習ロードマップ</h2></div><Link href="/roadmap">すべて見る <ArrowRight size={15}/></Link></div><div className="roadmap-preview-grid">{roadmaps.map((course,index)=><Link href={`/roadmap#${course.id}`} className="roadmap-preview-card" key={course.id} style={{'--course-color':course.accent} as React.CSSProperties}><span>{progress[index]>0?<CheckCircle2/>:<BookOpen/>}</span><div><h3>{course.title}</h3><p>{course.description}</p><div className="roadmap-progress"><i style={{width:`${progress[index]}%`}}/></div><small>{progress[index]}% 完了 ・ {course.modules.length}ステップ</small></div><ArrowRight size={18}/></Link>)}</div></section>
}
