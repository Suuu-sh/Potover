...
export function RoadmapPreview(){
  ...
  return <section className="roadmap-preview"><div className="modern-section-head"><HomeSectionHeading title="学習ロードマップ" note="3つのコースと進み具合"/><div className="roadmap-preview-actions">{!user&&<span className="roadmap-login-required"><LockKeyhole size={13} aria-hidden="true"/>ログイン必須</span>}<Link href={roadmapHref()}>すべて見る <ArrowRight size={15}/></Link></div></div><div className="roadmap-preview-grid">{roadmaps.map((course,index)=>{return <Link href={roadmapHref(course.id)} className="roadmap-preview-card home-elevated-card" key={course.id} style={{'--course-color':course.accent} as React.CSSProperties}><div><h3>{course.title}</h3><p>{course.description}</p><div className="roadmap-progress" role="progressbar" aria-label={`${course.title}の学習進捗`} aria-valuemin={0} aria-valuemax={100} aria-valuenow={progress[index]}><i style={{width:`${progress[index]}%`}}/></div><div className="roadmap-progress-caption"><small>{progress[index]}% 完了 ・ {course.modules.length}ステップ</small><small aria-hidden="true">100%</small></div></div><ArrowRight size={18}/></Link>})}</div></section>
}
