...
import {ArrowRight,Coins,Target,Trophy} from 'lucide-react';
...
export function RoadmapPreview(){
  ...
  return <section className="roadmap-preview"><div className="modern-section-head"><HomeSectionHeading eyebrow="LEARNING PATHS" title="学習ロードマップ"/><Link href="/roadmap">すべて見る <ArrowRight size={15}/></Link></div><div className="roadmap-preview-grid">{roadmaps.map((course,index)=>{const Icon=courseIcons[index];return <Link href={`/roadmap#${course.id}`} className="roadmap-preview-card home-elevated-card" key={course.id} style={{'--course-color':course.accent} as React.CSSProperties}><span><Icon aria-hidden="true"/></span><div><h3>{course.title}</h3><p>{course.description}</p><div className="roadmap-progress" role="progressbar" aria-label={`${course.title}の学習進捗`} aria-valuemin={0} aria-valuemax={100} aria-valuenow={progress[index]}><i style={{width:`${progress[index]}%`}}/></div><div className="roadmap-progress-caption"><small>{progress[index]}% 完了 ・ {course.modules.length}ステップ</small><small aria-hidden="true">100%</small></div></div><ArrowRight size={18}/></Link>})}</div></section>
}
