...
export default {async fetch(request:Request,env:Env){
  ...
  if(u.pathname==='/api/source-follows'&&request.method==='GET'){const user=await currentUser(request,env);if(!user)return json(request,{error:'ログインが必要です。'},{status:401});const {results}=await env.DB.prepare('SELECT source_slug FROM source_follows WHERE user_id=? ORDER BY created_at ASC').bind(user.id).all();return json(request,{sourceSlugs:(results as {source_slug:string}[]).map(row=>row.source_slug)})}
  if(u.pathname==='/api/source-follows'&&request.method==='POST'){const user=await currentUser(request,env);if(!user)return json(request,{error:'ログインが必要です。'},{status:401});let body:{sourceSlug?:unknown;followed?:unknown};try{body=await request.json()}catch{return json(request,{error:'入力内容を確認してください。'},{status:400})}const sourceSlug=typeof body.sourceSlug==='string'?body.sourceSlug.trim():'';if(!/^[a-z0-9-]+$/.test(sourceSlug))return json(request,{error:'ソースを確認できません。'},{status:400});if(body.followed===true)await env.DB.prepare('INSERT OR IGNORE INTO source_follows(user_id,source_slug) VALUES(?,?)').bind(user.id,sourceSlug).run();else if(body.followed===false)await env.DB.prepare('DELETE FROM source_follows WHERE user_id=? AND source_slug=?').bind(user.id,sourceSlug).run();else return json(request,{error:'フォロー状態を確認できません。'},{status:400});return json(request,{sourceSlug,followed:body.followed})}
