    ...
    <section className="curriculum-shell">
      <div className="curriculum-layout">
        ...
        <div className="curriculum-main">
          ...
          <section className="lesson-list"><header><h2>この章のレッスン</h2><span>{module.articles.filter(article=>read.has(article.slug)).length} / {module.articles.length} 完了</span></header>{module.articles.map((article,index)=><ArticleLink slug={article.slug} key={article.slug} className={read.has(article.slug)?'is-read':article.slug===nextArticle?.slug?'is-current':''}><span aria-hidden="true">{read.has(article.slug)?<Check/>:null}</span><strong>{activeModule+1}.{index+1}</strong><div><b>{article.title}</b><small>{article.source}</small></div><em><Clock3/>{article.minutes}分</em><ArrowRight/></ArticleLink>)}</section>
