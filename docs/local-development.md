...
本番の認証API・D1から分離して検証します。Wrangler 4を利用できるPATHで実行してください。
...
`Failed to fetch` はネットワークまたはCORSエラーです。フロントだけでなくAPIの起動、API URL、Originを確認してください。ローカル専用エントリは `localhost:3001` と `127.0.0.1:3001` のみ許可し、本番のCORSは変更しません。
